...
def display_all_images(file_path, encoded_samples, labels, grid_rows=28, grid_columns=20, tile_width=72,
                       tile_height=56, save=False, save_file_path=None, tsne=True, umap=False, pca=False):
    image_list = create_image_list(file_path=file_path)

    if tsne:
        results = Dimension_Reduction.TSNE_analysis(encoded_samples=encoded_samples, labels=labels, plot=False)
    elif umap:
        results = Dimension_Reduction.UMAP_analysis(encoded_samples=encoded_samples, labels=labels, plot=False)
    elif pca:
        results = Dimension_Reduction.PCA_analysis(encoded_samples=encoded_samples, labels=labels)
    else:
        raise Exception("No analysis method specified")

    # assign to grid
    grid_assignment = rasterfairy.transformPointCloud2D(results, target=(grid_rows, grid_columns))
    ...
    for img, grid_position in zip(image_list, grid_assignment[0]):
        ...
        if img_ar > aspect_ratio:
            ...
            img = img.crop((0, margin, img.width, margin + float(img.width) / aspect_ratio))
...
def display_image_distributions(image_file_path, dim_red_results, width=3000, height=3000, max_dim=100):
...
# show_cluster_data(pca_df_clusters, cluster = 0)
def create_image_list(file_path, transform=None):
    full_dataset = Image_Loader.generate_dataset(folder_path=file_path, transform=transform)
    image_list = []

    trnsform = torchVisionTransforms.ToPILImage()
...
def show_cluster_images(image_file_path, cluster_results, show_cluster_number, grid_rows=5, grid_columns=5,
                        fig_size=(10, 10), all=False, test=False):
    """ Prints the cluster data in a grid"""

    image_list = create_image_list(file_path=image_file_path)
    dataframe = pd.concat([pd.Series(image_list), pd.DataFrame({'cluster': cluster_results})], axis=1)
    hash_map = {}
    ...
    if test:
        dict_clust = Image_Loader.show_classes(folder_path=image_file_path, transform=None)
        res = dict((v, k) for k, v in dict_clust.items())
        fig.suptitle(" %s-images , total images in cluster = %s " % (res[target_name], len(cluster_df)))
    else:
        fig.suptitle("Cluster number %s , images in cluster = %s " % (target_name, len(cluster_df)))
